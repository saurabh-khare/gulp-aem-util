 ...
 * @param {String} dest
 */
var getConfigCollection = function(files, src, base, dest) {
  var fileCollection = [];
  _.each(files, function(file) {
    try {
      ...
      if (e.code === "ENOENT" || e.code === "EISDIR") {
       console.warn(file + "Is either a directory or does not exists");
      }
   }
  });
 ...
 * @returns {[], []}
 */
var generateClientLibFiles = function writeClientLibFiles(clientLibs, libroot) {
  var content;
  var files;
  var libFiles = {};
      ...
      if (clientLibs[name].js.length > 0) {
        _.each(clientLibs[name].js, function(fileObj) {
          ...
          libFiles.fileCollection = _.union(libFiles.fileCollection,
            getConfigCollection(files, fileObj.path, libroot, libroot + name + "/js"));
        });
        content.trim();
        var jsFile = new File({
          base: libroot,
          cwd: libroot,
          path: libroot + name + "/js.txt",
    ...
    if (clientLibs[name].hasOwnProperty("css")) {
      if (clientLibs[name].css.length > 0) {
        ...
        _.each(clientLibs[name].css, function(fileObj) {
          ...
          content += "\n";
          libFiles.fileCollection = _.union(libFiles.fileCollection,
            getConfigCollection(files, fileObj.path, libroot, libroot + name + "/css"));
        });
        content.trim();
        var file = new File({
          base: libroot,
          cwd: libroot,
          path: libroot + name + "/css.txt",
          contents: new Buffer(content)
        });
    ...
    if (clientLibs[name].hasOwnProperty("resources")) {
      if (clientLibs[name].resources.length > 0) {
        _.each(clientLibs[name].resources, function(fileObj) {
          ...
          files = expand(expandOptions, fileObj.files);
          libFiles.fileCollection = _.union(libFiles.fileCollection,
            getConfigCollection(files, fileObj.path, libroot, libroot + name + "/resources"));
        });
      }
    }
  });
